    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Tambah Pemasukan</h1>
                    <p class="mt-2 text-gray-600">Tambah data pemasukan baru</p>
                </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900">Form Pemasukan</h3>
            </div>
            
            <form action="{{ route('admin.management.finance.income.store.simple') }}" method="POST" class="p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="class" name="class" required
                            ...
                            <option value="SD 1" {{ old('class') == 'SD 1' ? 'selected' : '' }}>SD 1</option>
                            <option value="SD 2" {{ old('class') == 'SD 2' ? 'selected' : '' }}>SD 2</option>
                            <option value="SD 3" {{ old('class') == 'SD 3' ? 'selected' : '' }}>SD 3</option>
                            <option value="SD 4" {{ old('class') == 'SD 4' ? 'selected' : '' }}>SD 4</option>
                            <option value="SD 5" {{ old('class') == 'SD 5' ? 'selected' : '' }}>SD 5</option>
                            <option value="SD 6" {{ old('class') == 'SD 6' ? 'selected' : '' }}>SD 6</option>
                            <option value="SMP 1" {{ old('class') == 'SMP 1' ? 'selected' : '' }}>SMP 1</option>
                            <option value="SMP 2" {{ old('class') == 'SMP 2' ? 'selected' : '' }}>SMP 2</option>
                            <option value="SMP 3" {{ old('class') == 'SMP 3' ? 'selected' : '' }}>SMP 3</option>
                            <option value="SMA 1" {{ old('class') == 'SMA 1' ? 'selected' : '' }}>SMA 1</option>
                            <option value="SMA 2" {{ old('class') == 'SMA 2' ? 'selected' : '' }}>SMA 2</option>
                            <option value="SMA 3" {{ old('class') == 'SMA 3' ? 'selected' : '' }}>SMA 3</option>
                        </select>
                        @error('class')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="receipt_number" name="receipt_number" value="{{ old('receipt_number') }}" required
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors duration-200 @error('notes') border-red-500 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
